...
def signal(symbol):
    ...
    print(strftime("%H:%M:%S",  localtime()), symbol + ' | ' + 
    output.summary['RECOMMENDATION'], "| N/B/S :", 
    round((100/26)*output.summary['NEUTRAL'], 2), "% /", 
    round((100/26)*output.summary['BUY'], 2), "% /", 
    round((100/26)*output.summary['SELL'], 2), "%")
    signal(symbol)
...
if __name__ == "__main__":
    ...
    coin, interval, limit, submitted = chart_settings(coins)
    if submitted: 
        visualize(candles(coin, interval, int(limit)))
        signal(coin)
